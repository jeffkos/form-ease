...
class SMSService {
    ...
    try {
      ...
      if (process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN) {
        try {
          ...
          logger.warn(
            "Twilio module not found, skipping SMS provider initialization"
          );
        }
      }

      // Configuration MailerSend SMS (optionnel)
      if (process.env.MAILERSEND_SMS_API_KEY) {
        try {
          ...
          logger.warn(
            "MailerSend module not found, skipping SMS provider initialization"
          );
        }
      }

      // Configuration Free Mobile (gratuit pour tests)
      if (process.env.FREE_MOBILE_USER && process.env.FREE_MOBILE_PASS) {
        // Free Mobile ne nécessite pas de module externe
      ...
      // Si aucun provider n'est configuré, utiliser un mode simulation
      if (Object.keys(this.providers).length === 0) {
        this.providers.simulation = {
          mode: "development",
        };
        logger.info(
          "SMS service running in simulation mode (no real SMS will be sent)"
        );
      }
  ...
  async sendSMS({ to, message, formId = null, userId = null, metadata = {} }) {
    try {
      ...
      if (!validation.isValid) {
        throw new Error(
          `Données SMS invalides: ${validation.errors.join(", ")}`
        );
      }
      ...
      switch (this.activeProvider) {
        ...
        case "freemobile":
          ...
          break;
        case "simulation":
          result = await this.sendViaSimulation(normalizedPhone, message);
          break;
        default:
          // Fallback vers simulation si provider non supporté
          logger.warn(
            `Provider SMS non supporté: ${this.activeProvider}, utilisation du mode simulation`
          );
          result = await this.sendViaSimulation(normalizedPhone, message);
          break;
      }
      ...
      if (to && message) {
        await this.saveSMSRecord({
          ...
          logger.error(
            "Erreur lors de l'enregistrement de l'échec SMS:",
            dbError
          );
        });
      }
  ...
  async sendBulkSMS({
    ...
    logger.info(
      `Début d'envoi SMS en lot pour ${recipients.length} destinataires`
    );
    ...
    logger.info(
      `Envoi SMS en lot terminé: ${results.length} succès, ${errors.length} échecs`
    );
  ...
  async sendViaFreeMobile(to, message) {
    ...
    const fetch = require("node-fetch");
    const { user, pass } = this.providers.freemobile;
    ...
    const response = await fetch(
      `https://smsapi.free-mobile.fr/sendmsg?${params}`
    );
  ...
  async getSMSHistory(filters = {}) {
    try {
      ...
      logger.error(
        "Erreur lors de la récupération de l'historique SMS:",
        error
      );
      throw error;
    }
