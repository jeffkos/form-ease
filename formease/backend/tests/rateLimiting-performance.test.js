...
describe("⚡ Rate Limiting - Tests de performance et stress", () => {
  ...
  describe("🚀 Tests de performance", () => {
    ...
    test.skip("ipWhitelist devrait être très rapide", () => {
      // Test désactivé temporairement - performance variable selon l'environnement
      process.env.WHITELISTED_IPS = Array.from(
        { length: 1000 },
        (_, i) => `192.168.1.${i}`
      ).join(",");
    ...
    test("Concurrent access au store en mémoire", async () => {
      ...
      for (let i = 0; i < numOperations; i++) {
        concurrentOperations.push(
          new Promise((resolve) => {
            setTimeout(() => {
              global.__rateLimitStore[key] =
                (global.__rateLimitStore[key] || 0) + 1;
              resolve();
            }, Math.random() * 10);
          })
        );
  ...
  describe("📊 Tests de monitoring des performances", () => {
    test("Mesure du throughput de smartRateLimit", () => {
      ...
      console.log(
        `⏱️ Latence moyenne: ${(durationMs / requestCount).toFixed(3)}ms`
      );
    });
    ...
    test("Profiling de la mémoire du store", () => {
      ...
      console.log(
        `📈 Augmentation totale: ${(memoryIncrease / 1024 / 1024).toFixed(
          2
        )} MB`
      );
