...
describe('🛡️ Rate Limiting Middleware - Tests complets', () => {
  ...
  describe('🏭 createRateLimiter(type)', () => {
    ...
    test('Devrait lancer une erreur pour un type invalide', () => {
      expect(() => createRateLimiter('invalid')).toThrow('Unknown rate limit type: invalid');
    });
  ...
  describe('🔐 bruteForceDetection()', () => {
    ...
    test('Devrait incrémenter le compteur pour les endpoints d\'auth', async () => {
      ...
      expect(mockRedisClient.expire).toHaveBeenCalledWith('bf:192.168.1.1', 3600);
      expect(next).toHaveBeenCalled();
    });
  ...
  describe('🚪 closeRedis()', () => {
    test('Devrait fermer la connexion Redis si connectée', async () => {
      ...
      const originalGetRedisClient = require('../src/middleware/rateLimiting').getRedisClient;
